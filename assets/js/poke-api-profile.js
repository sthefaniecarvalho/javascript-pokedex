...
function convertPokemonApiAboutDetailToPokemon(pokeDetail) {
...
function convertPokemonApiAboutSpecieDetailToPokemon(pokeSpecie, pokemon){
    pokemon.gender = getGenderPercentages(pokeSpecie.gender_rate)
    pokemon.egg_groups = pokeSpecie.egg_groups.map((egggroup) => egggroup.name)[0]
    pokemon.egg_cycle = pokemon.types[0]
...
pokeApiAbout.getPokeGender = (pokemon) => {
    return fetch(pokemon.species.url)
        ...
        .then((jsonBody) => convertPokemonApiAboutSpecieDetailToPokemon(jsonBody, pokemon) ))
}

pokeApiAbout.getPokemonDetail = (pokemon) => {
    return fetch(pokemon.url)
        .then((response) => response.json())
        .then(convertPokemonApiAboutDetailToPokemon)
